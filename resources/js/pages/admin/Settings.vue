...
const getDisplayValue = (setting: Setting): string => {
    ...
    if (setting.field_type === 'multioptions' && setting.options) {
        const options = setting.options.split(',').map((opt) => opt.trim()).filter(Boolean);
        const currentValue = setting.value?.toString() ?? '';
        return options.includes(currentValue) ? currentValue : (setting.value?.toString() ?? 'N/A');
    }

    return setting.value?.toString() ?? 'N/A';
};

const deleteSetting = (settingId: number, settingKey: string, role: string): void => {
    if (role === 'system') {
                        ...
                        <div class="flex items-start justify-between">
                            <div class="flex-1 space-y-2">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-xs text-muted-foreground">({{ setting.key }})</span>
                                    <span
                                        :class="{
                                            'bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400': setting.role === 'system',
                                            'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400': setting.role === 'user',
                                            'bg-purple-100 text-purple-800 dark:bg-purple-900/20 dark:text-purple-400': setting.role === 'plugin',
                                        }"
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm">{{ getDisplayValue(setting) }}</span>
                                </div>
                            </div>
                            <div class="flex items-center gap-2">
                                <Link
                                ...
                                <button
                                    ...
                                    @click="deleteSetting(setting.id, setting.key, setting.role)"
                                    :class="{
                                        'text-sm text-destructive hover:underline': setting.role !== 'system',
                                        'text-sm text-muted-foreground cursor-not-allowed': setting.role === 'system',
                                    }"
                                >
                                    Delete
                                </button>
