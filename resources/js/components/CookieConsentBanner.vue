...
import { Settings, Cookie, Shield, BarChart3, Target } from 'lucide-vue-next'
...
<template>
  ...
  >
    <div
      ...
      class="fixed bottom-0 left-0 right-0 z-[9999] bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-t shadow-lg"
    >
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div class="flex-1 space-y-2">
            ...
            <p class="text-sm text-muted-foreground">
              {{ config.banner?.description || 'We use cookies to enhance your browsing experience.' }}
            </p>
          ...
          <div class="flex flex-col gap-2 sm:flex-row">
            ...
            >
              {{ config.banner?.buttons?.reject_all || 'Reject All' }}
            </Button>
            ...
            <Button
              ...
              :disabled="isProcessing"
            >
              {{ config.banner?.buttons?.accept_all || 'Accept All' }}
            </Button>
          </div>
        </div>
      </div>
  ...
  <Dialog v-model:open="showModal">
    <DialogContent class="max-w-2xl max-h-[80vh] overflow-y-auto">
      <DialogHeader>
        ...
        <DialogDescription>
          {{ config.modal?.description || 'Manage your cookie preferences.' }}
        </DialogDescription>
      </DialogHeader>
        ...
        <Card>
          <CardHeader>
            <CardTitle class="flex items-center gap-2">
              ...
              {{ config.modal?.sections?.essential_title || 'Essential Cookies' }}
            </CardTitle>
            <CardDescription>
              {{ config.modal?.sections?.essential_description || 'These cookies are necessary for the website to function.' }}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div class="space-y-3">
        ...
        <Card>
          <CardHeader>
            <CardTitle class="flex items-center gap-2">
              ...
              {{ config.modal?.sections?.optional_title || 'Optional Cookies' }}
            </CardTitle>
            <CardDescription>
              {{ config.modal?.sections?.optional_description || 'You can choose which optional cookies to allow.' }}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div class="space-y-3">
