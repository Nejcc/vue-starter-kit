...
const loadPreferences = (): void => {
    try {
        ...
        console.warn(
            'Failed to load cookie preferences from localStorage:',
            error,
        );
    }
};
...
const savePreferences = (preferences: Record<string, boolean>): void => {
    try {
        ...
        console.warn(
            'Failed to save cookie preferences to localStorage:',
            error,
        );
    }
};
...
const rejectAll = async (): Promise<void> => {
    ...
    const preferences = Object.fromEntries(
        categories.map((category) => [
            category,
            category === 'essential' ||
                cookieConsentState.value.categories[category]?.required ||
                false,
        ]),
    );
...
watch(hasConsent, (newValue) => {
    if (newValue) {
        ...
        window.dispatchEvent(
            new CustomEvent('cookieConsentGiven', {
